...
class Operations():
    ...
    def table_dump(self, inp_date, center, session, p_table):
            a_slots=''
            if 'slots' in session:
    ...
    def do_telegram(self, data):
        telegram_token = '1837098146:AAHsEsRJVZrm1vhzHQRxYufubSdjfXO9Kj0'
        telegram_chat_id = '@CoWinVacIndia'
        alert_text = data
        URL = "https://api.telegram.org/bot{0}/sendMessage?chat_id={1}&text={2}".format(telegram_token,telegram_chat_id, alert_text)
    ...
    def do_whatsapp(self, data):
        ...
        client.messages.create(body=data,
    ...
    def process_data(self, date_str):
            ...
            for inp_date in date_str:
                URL = "https://cdn-api.co-vin.in/api/v2/appointment/sessions/public/calendarByPin?pincode={0}&date={1}".format(self.args.pincode, inp_date)
                ro = RestOperations(self.args)
                ...
                if response:
                    for center in response["centers"]:
                        for session in center["sessions"]:
                            if int(session["available_capacity"]) > 0 and int(session["min_age_limit"]) <= int(self.args.age):
                                self.table_dump(inp_date, center, session, p_table)
...
class RestOperations(Operations):
    ...
    """
        if args.verbose: 
            print('Requested url : %s, headers = %s, params = %s, expected return code = %s' %(url, headers, params, expected_return_code))
        try:
